...
## 7.2 Supervisor 잠정 판정 (가설 B Fallback — cloud refine 실패 시 정식 채택)

> **상태**: cloud `/ultraplan refine plan.md` (session_014GGwaNRmFuBjhzyMtduAs9) 가 가설 A (workspace=ruler) 로 응답하면 본 §7.2 를 cloud 결과로 **덮어씀**. 가설 B (workspace=Button 고정) 로 응답하면 본 §7.2 가 정식 채택.
>
> **판정 주체**: Supervisor (현 세션 95% tok, Opus 전환 §E 위반 불가 → 잠정 판정만 기록).

### Q1. Python (change-impact.py) 제거 — **채택: Cost 권고 (bash+jq 통합)**

- **결정**: `scripts/change-impact.py` 신설 취소. `scripts/retrospective-collect.sh` 에 `compute_change_impact()` bash 함수 추가 + jq 로 verdict 계산.
- **근거**: (a) ruler patrol/batch 양쪽 모두 bash + jq 만 사용 중 (Python 의존성 0) (b) decisions.jsonl 4일치 551건 규모에서 jq 파싱 충분 (c) 단일 스크립트 = 단일 syscall = patrol 의 3분 wake 루프 부담 최소.
- **trade-off 인식**: 5번째 INSUFFICIENT 게이트 (Poisson 신뢰구간) 는 jq 로 표현 어색 → bash 함수 내부에서 단순 `if N<10` 로 단순화. 정밀도 손실 수용.
- **plan §5 #4 갱신**: "신규 `change-impact.py`" → "**기존 `retrospective-collect.sh` 에 `compute_change_impact()` 함수 추가**". progress.md Step 4 도 동일 갱신 필요.

### Q2. Phase A 활성화 시점 — **채택: 절충안 (구현 + 4주 관찰 전용)**

- **결정**: Phase A 구현은 본 plan 범위에서 완료 (Step 1+4). 단, **첫 4주 (2026-04-18 ~ 2026-05-16) 는 `verdict_observation_only: true` 모드** — verdict 산출 + `retrospective/{date}_change-impact.md` 기록은 하되 **preflight 승격 / pending 큐 / handoff 트리거는 차단**.
- **근거**: (a) Cost 권고 핵심 (4주 데이터 부족 → INSUFFICIENT 폭주) 인정 (b) 사용자 Primary 요구 (인과 판정) 를 4주 늦추는 건 거부 — 첫날부터 verdict 산출은 시작 (c) 단, 초기 verdict 의 신뢰도 부족 → handoff 차단으로 오탐 사고 방지.
- **구현 위치**: `change-impact.py` (또는 `compute_change_impact()` bash 함수) 의 출력 md 상단에 `> ⚠️ OBSERVATION-ONLY MODE (until 2026-05-16)` 배너. Phase Terminal 의 pending 생성 분기에서 `if observation_only: skip pending creation`.
- **종료 조건**: 2026-05-16 retrospective 에서 Supervisor 가 verdict 분포 (GOOD/BAD/INSUFFICIENT 비율) 검토 → INSUFFICIENT < 50% 시 observation 해제. 이상이면 +2주 연장.

### Q3. R1~R11 verdict 통합 — **채택: (a) Cost 축소안**

- **결정**: R1~R11 정의는 `retrospective-guide.md §부록` 으로 이동 (plan §3.1 그대로). Phase A 가 BAD verdict 산출 시 출력 md 의 `Δ summary` 칼럼에 **"R# pattern X hits"** 1줄 보조 표기.
- **근거**: (a) Phase A verdict 본체는 `decisions.jsonl` Δ 만으로 결정 (튜닝 포인트 단순화) (b) R1~R11 빈도 패턴은 BAD 의 **근거 강화 재료** 로만 사용 — 사문화 방지 + verdict 본체 오염 회피 (c) DA 편입안 (verdict 표 5번째 행 "soft-BAD") 은 임계값 2개 (Δ + R# 빈도) 동시 튜닝 → 첫 4주 관찰 모드와 충돌.
- **구현 위치**: `change-impact.py` (또는 bash 함수) 의 verdict=BAD 분기에서 R1~R11 `--window 7d` 결과 파일 읽어 해당 file/check 매칭되는 R# 만 추출. 매칭 없으면 보조 표기 생략.

### §6.5 보완 사항

- **§6.5.5 (change-impact.py 합성 fixture 의무화)**: Q1 채택으로 Python 제거되었으므로, fixture 검증은 `compute_change_impact()` bash 함수 + bats 또는 shell assert harness 로 대체. 합성 fixture 3종 (GOOD/BAD/INSUFFICIENT) 의무는 유지.
- **§6.5.4 (외부 skill drift)**: `external-skill-checksums.md` 신규 파일 방식 그대로. progress.md Step 5/6 직후 ruler Tier C 가 이 파일을 읽도록 patrol-tier-c.md §C_external 항목 추가 필요 (별도 step 으로 분할).

### progress.md 영향 (가설 B 채택 시 갱신)

| Step | 변경 |
|------|------|
| Step 4 | "`scripts/change-impact.py` 신규 작성 (model: opus)" → "**`scripts/retrospective-collect.sh` 에 `compute_change_impact()` bash 함수 추가 (model: sonnet)**". model 강등 + 별도 신규 파일 제거. |
| Step 1 | retrospective-guide §3.2 verdict 절차에 `verdict_observation_only` 플래그 명시. R1~R11 보조 표기 절차 추가. (변경 소량 — opus 유지) |
| Step 7 (Smoke test) | `합성 fixture 주입 dry-run` 으로 변경 (실데이터 baseline=0 승격 → verdict 추출 검증 불가). §6.5.5 와 동기화. |
| Step 8 (신규) | `patrol-tier-c.md §C_external` 항목 추가 (model: sonnet, 1 line append). |

---

<!-- PHASE-2α-SKIP: 2026-04-18T02:50:00Z reason=cloud_workspace_locked_to_button hypothesis=B -->

## 7.2 Supervisor 판정 (§7.1 DEFER 3건 로컬 결정)

원격 ultraplan 은 cloud workspace 고정 (Button WOL) 으로 ruler repo 접근 불가. Blocker 보고서만 반환 → 로컬 Supervisor(Opus) 직접 판정 수행.

### Q1 — Python (change-impact.py) 제거 ✅ Cost 권고 채택

- **결정**: `.ruler/scripts/change-impact.py` 신규 계획 **취소**. `.ruler/scripts/retrospective-collect.sh` 에 bash 함수로 통합.
- **근거**: 런타임 경로 3종 (`py -3`/`python3`/절대) 부담 vs datetime 파싱 이득 미미. bash+jq+awk+comm 로 동등 계산 가능. 단일 스크립트 유지보수 이득 > 150줄 Python 분리.
- **영향**: plan §5 편집 대상 표 #4 삭제. progress.md Step 4 삭제, Step 3 범위 확장.

### Q2 — Phase A 활성화 시점: 절충안 (관찰 전용 4주)

- **결정**: Phase A 구현은 **1차 배포 즉시**. 초회 4주간 **관찰 전용 모드** — verdict 산출하되 `preflight 승격 금지` + `pending/revert-*.md 생성 금지`. 순수 GOOD/BAD/INSUFFICIENT 분포 관찰.
- **근거**: 사용자 Primary 요구 (인과 판정) 4주 연기는 scope 축소. 반면 Cost 의 "초회 판정 가능 건수 20-30, 대부분 INSUFFICIENT" 우려는 유효 → 불완전 verdict 가 실행 행위까지 유도하지 않도록 4주 격리.
- **구현**: `.ruler/state.md` 에 `change_impact_enforcement_start: 2026-05-16` (4주 후) 필드 추가. change-impact 로직에서 이 date 이전이면 verdict 산출+기록만, preflight/revert handoff skip.

### Q3 — R1~R11 통합 방식: (a) Cost 축소안

- **결정**: R1~R11 부록 유지 + Phase A BAD 판정 시 "R# 빈도 급등 보조 확인" 1줄 추가.
- **근거**: DA 편입안(5번째 판정행)은 튜닝 포인트 증가 → 초기 데이터 부족 상태에서 오탐 위험. 폐기는 기존 정보 손실. 축소안이 최소 리스크.
- **구현**: retrospective-guide.md 재작성 시 §Phase A BAD verdict 기준 4번째 행 밑에 "보조: R1~R11 해당 패턴 빈도가 동 window 대비 +50%+ 이면 BAD 확증" 1줄 추가.

...
last-edit: 2026-04-18 btn-ruler-step6-agent Phase-Final-B-redefine
---
...
### Phase Final-B — Ruler Phase B 보완 감사 [2026-04-18 재정의, Phase Final 직후]

**역할 재정의 (2026-04-18, progress.md Step 6)**: 본 Phase 는 **ruler-batch Phase B (Compliance Audit) 의 보완 감사** 다. ruler Phase B 가 다루지 못하는 경계 밖 범위 — **장기 drift / 파일 리스트 추적 / 인덱스 정합성 / rollback 품질 저하 / 7일 window 밖 누적 drift** — 에 한정하여 실행한다.

**⛔ 중복 회피 — 단일 소유자 원칙**: **최근 7일 §0.5 누락 감사 는 ruler-batch Phase B 가 단일 소유자** (`D:/projects/ruler/.ruler/retrospective-guide.md` §Phase B). 본 Phase Final-B 는 **7일 window 내 §0.5 누락 감지·backfill 로직에 진입하지 않는다**. 같은 범위를 두 번 훑으면 decisions.jsonl 에 중복 `backfill` entry + 서로 다른 verdict 가 쌓여 Phase A 재평가가 오염된다.

**발동 조건** (AND — 모두 만족 시에만 진입):
- 본 세션이 **ruler-batch Phase B 실행 세션이 아님** (메인 대화의 `정기 감사` / `감사 wf` / `hook 검증` 인라인 호출) — ruler-batch Phase B 에서 호출된 경우 본 Phase 는 skip (Phase B 가 이미 §0.5 감사 완료)
- OR **7일 window 밖 장기 drift 신호**: `decisions.jsonl` 에 30일+ 미갱신 `rules/`·`skills/` 참조, 혹은 `MEMORY.md` 인덱스 ↔ `memory/**/*.md` 파일 리스트 단절 탐지

**절차 (7일 window 밖 drift 만)**:
1. `decisions.jsonl` 에서 **30일+ 장기 구간** `file:` 가 `rules/` / `skills/` 인 항목 수집 (7일 이하는 ruler Phase B 소관이므로 제외)
2. 각 항목에 대해: 해당 규칙을 감시하는 patrol 체크 식별 (`grep -l`) — 단, **patrol.md / event-rules.yaml / rules/*.md 로 좁혀진 의미 drift** 는 ruler Phase B Step 3 가 이미 처리하므로 **파일 리스트 추적 / 인덱스 정합성** 층위만 다룬다
3. 장기 drift 감지: (a) 규칙 삭제·이동인데 레퍼런스가 남아있음 (b) 인덱스 (SKILL_INDEX / MEMORY / *_INDEX.md) 에 존재하지만 실제 파일 부재 (c) rollback 누적 비율 (최근 100건 기준) 이 ruler B4 threshold (10%) 를 넘지만 개별 rollback 이 ruler Phase B 7일 window 밖에 분포
4. 수정안 작성 + `decisions.jsonl` append — `action:"long_term_drift_sync"`, `reason` 필드에 7일 window 밖인 근거 명시 (`"30d_stale"` / `"index_orphan"` / `"rollback_cumulative"`)
5. 불일치 N건 + 수정 M건 + **skip 사유** ("within ruler Phase B window" count) 요약 보고

**SSOT**: 규칙 파일 (`rules/`, `skills/`) 이 우선. 단, **7일 내 §0.5 / patrol drift** 판정은 ruler Phase B 가 SSOT (본 Phase 는 관측만, Edit 금지).

**위임 경계 교차 참조**: `D:/projects/ruler/.ruler/retrospective-guide.md` Phase C §경계 블록 (L357) 이 본 Phase Final-B 에 "7일 밖 drift / 파일 리스트 / 인덱스 / rollback 품질 저하" 를 위임함을 명시. 양방향 링크로 경계 drift 차단.
